...
def task_4b_implementation(sim):
	...
	for i in res:
		...
		if lens >= 3 and u == 2:
			...
			drop(location1,words[0])
			traversel(path,location1,location2)
			drop(location,words[1])
			traversel(path,location2,location3)
			drop(location,words[2])
			if lens == 4:
				traversel(path,location3,'F2')
			else:
				traversel(path,location3,'end_node')
			flag=1
		if u==3:
			traversel(path,'F2',location1)
			drop(location1,words[0])
			traversel(path,location1,'end_node')
			flag=1
